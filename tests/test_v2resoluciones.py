...
class TestLearningReportAPI(unittest.TestCase):
    def setUp(self):
        self.tmp = temp_dir()
        self.api = ServeAPI(self.tmp.name)
        self.addCleanup(self.api.close)
        self.addCleanup(self.tmp.cleanup)

    def test_informe_requiere_autorizacion(self):
        self.api.users.authenticate = Mock(return_value=(None, None))
        code, result = self.api.handle("GET", "/api/learning-report", {}, b"")
        self.assertEqual(code, 401)
        self.assertIn("credencial", result["error"])

    def test_informe_vacio_usa_workspace_del_usuario(self):
        self.api.users.authenticate = Mock(return_value=("ana", "viewer"))
        code, result = self.api.handle(
            "GET", "/api/learning-report?limit=10",
            {"Authorization": "Bearer token"}, b"")
        self.assertEqual(code, 200)
        self.assertEqual(result["history"]["records"], [])
        self.assertEqual(result["metrics"]["attempts"], 0)
        self.assertEqual(result["recommendations"], [])

    def test_informe_devuelve_exactamente_learning_report_persistido(self):
        from a2s.autonomy import AutonomousLoop
        self.api.users.authenticate = Mock(return_value=("ana", "viewer"))
        loop = AutonomousLoop(self.api.runner.user_workspace("ana"))
        record = {"mission_id": "mission-a", "objective": "consulta",
                  "selected_tools": [{"source": "alpha"}], "decision": "accept",
                  "cost": 2, "result": {"before": 1, "after": 3}}
        (loop.missions_dir / "mission-a.json").write_text(
            json.dumps(record), encoding="utf-8")
        expected = loop.learning_report(objective="consulta", limit=10)
        code, result = self.api.handle(
            "GET", "/api/learning-report?objective=consulta&limit=10",
            {"Authorization": "Bearer token"}, b"")
        self.assertEqual(code, 200)
