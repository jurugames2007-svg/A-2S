...
class TestAutomaticEvolutionConfig(unittest.TestCase):
    def test_auto_evolve_is_off_by_default(self):
        with mock.patch.dict(os.environ, {}, clear=True):
            self.assertFalse(Config().auto_evolve)

    def test_auto_evolve_can_be_enabled_explicitly(self):
        with mock.patch.dict(os.environ, {"A2S_AUTO_EVOLVE": "1"}, clear=True):
            self.assertTrue(Config().auto_evolve)
