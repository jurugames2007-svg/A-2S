...
class TestEnrutador(unittest.TestCase):
    ...
    def test_rutas_de_integracion_cubren_fuentes_del_prompt(self):
        casos = {
            "navegar web": {"auto-browser", "crawl4ai"},
            "conversacion por voz": {"speech-to-speech"},
            "auditoria seo": {"claude-seo"},
            "crear apk android": {"webtoapp"},
            "buscar api publica": {"public-apis"},
            "gestionar procesos pm2": {"pm2"},
            "convertir documento a skill": {"book-to-skill"},
        }
        for objetivo, esperados in casos.items():
            with self.subTest(objetivo=objetivo):
                ids = {paso["id"] for paso in seleccionar(objetivo)["pasos"]}
                self.assertTrue(esperados <= ids)

    def test_rutas_de_integracion_no_abren_explotacion(self):
        plan = seleccionar("navegar web y explotar con metasploit")
        ids = {paso["id"] for paso in plan["pasos"]}
        bloqueados = {paso["id"] for paso in plan["bloqueados"]}
        self.assertNotIn("metasploit", ids)
        self.assertIn("metasploit", bloqueados)

    def test_adaptadores_nativos_no_quedan_como_recurso_generico(self):
        self.assertEqual(resolver("pm2").capacidad, "gestion_procesos_locales")
        self.assertEqual(resolver("public-apis").capacidad, "catalogo_apis_https")
        self.assertEqual(resolver("yt-dlp").uso, "parcial")
        self.assertEqual(resolver("orca").capacidad, "orquestacion_local")
