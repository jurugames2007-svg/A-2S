...
(`signing`). También existen adaptadores nativos acotados para procesos y
APIs (`integrations.py`), crawling/SEO/skills (`web_integrations.py`) y
metadatos multimedia/orquestación (`media_orchestration.py`). Las fuentes de
voz, GUI, Android, WiFi, ML profundo y publicación social siguen siendo
`operador` porque requieren dependencias, hardware, cuentas o confirmación
explícita; no se presentan como funciones ya operativas.
