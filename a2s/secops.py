...
def _run_scanner(argv: list[str], workspace: str, timeout: float) -> dict[str, Any]:
    """Ejecuta un escáner local por la única frontera de procesos de A²S."""
    sandbox = Sandbox(workspace or ".", allow_network=False,
                     timeout=max(1, int(timeout)))
    result = sandbox.run_cmd(argv, timeout=max(1, int(timeout)))
    return {
        "stdout": result.stdout or "",
        "stderr": result.stderr or "",
        "returncode": result.returncode,
        "timed_out": result.timed_out,
        "sandbox_level": result.level,
        "sandbox": result.level_name,
    }
...
def ejecutar_nuclei(targets: list[str], templates: str = "",
                    timeout: float = 120.0, workspace: str = "") -> dict[str, Any]:
    ...
    for target in targets:
        ...
        if templates:
            argv += ["-t", templates]
        try:
            proc = _run_scanner(argv, workspace or ".", timeout)
    ...
    return {"ok": not errores or bool(findings), "findings": findings,
            ...
            "sandbox": proc.get("sandbox", "") if 'proc' in locals() else ""}
