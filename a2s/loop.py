...
class AgentLoop:
    ...
    def _finalize(self, goal: str, achieved: bool, reason: str, deadline: float) -> RunReport:
        ...
        generations = self.config.evolve_generations or (5 if self.config.auto_evolve else 0)
        if generations > 0 and self.neural is not None:
            try:
                from .neuroevolve import evolve_from_memory
                evolve_from_memory(self.memory, generations=generations,
                                   target=os.path.join(self.memory.dir, "governance.json"))
                self.memory.ledger.append("neuroevolution",
                                          {"generations": generations,
                                           "automatic": self.config.auto_evolve})
