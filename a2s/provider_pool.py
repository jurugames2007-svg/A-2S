...
def _emit_task_event(event_sink: Optional[Callable[[dict[str, Any]], None]],
                     run_id: str, task_id: str, status: str, **extra: Any) -> None:
    if event_sink is None:
        return
    event = {"event": "task", "status": status, "run_id": run_id,
             "task_id": task_id, "at": now_iso(), **extra}
    try:
        event_sink(event)
    except Exception:
        pass
...
def _record_dag_future(future: Any, task_id: str,
                       results: dict[str, Optional[str]], failed: set[str],
    ...
    try:
        ...
        failed.add(task_id)
        _emit_task_event(event_sink, run_id, task_id, "error",
                         error=f"{type(exc).__name__}: {exc}")
        return
    if results[task_id] is None:
        failed.add(task_id)
        _emit_task_event(event_sink, run_id, task_id, "error",
                         error="proveedor sin resultado")
    else:
        _emit_task_event(event_sink, run_id, task_id, "done")


def _dag_batch_plan(batch: list[str], by_id: dict[str, dict[str, Any]],
                    deps: dict[str, list[str]], failed: set[str]
                    ) -> tuple[list[dict[str, Any]], list[str]]:
    runnable = [by_id[tid] for tid in batch
                if not any(dep in failed for dep in deps[tid])]
    runnable_ids = {task["id"] for task in runnable}
    return runnable, [tid for tid in batch if tid not in runnable_ids]


def _dag_should_stop(stop: Any, deadline: Optional[float]) -> bool:
...
class ProviderPool(BaseProvider):
    ...
    def _execute_dag_batch(self, runnable: list[dict[str, Any]], max_par: int,
                           results: dict[str, Optional[str]], failed: set[str],
                           deadline: Optional[float], stop: Any,
                           event_sink: Optional[Callable[[dict[str, Any]], None]],
                           run_id: str) -> tuple[set[str], bool]:
        """Ejecuta una ola y devuelve (canceladas, debe_parar)."""
        for task in runnable:
            _emit_task_event(event_sink, run_id, task["id"], "running")
        cancelled: set[str] = set()
        with ThreadPoolExecutor(max_workers=min(max_par, len(runnable))) as pool:
            futures = {pool.submit(self.chat, task["prompt"], task.get("kind", "general"),
                                   task.get("max_tokens", 1000), SYSTEM_PROMPT, False): task["id"]
                       for task in runnable}
            try:
                remaining = None if deadline is None else max(0.0, deadline - time.monotonic())
                for future in as_completed(futures, timeout=remaining):
                    _record_dag_future(future, futures[future], results, failed,
                                       event_sink, run_id)
            except FuturesTimeoutError:
                cancelled.update(tid for future, tid in futures.items()
                                 if tid not in results)
                for future in futures:
                    future.cancel()
            should_stop = stop is not None and stop.is_set()
            if should_stop:
                cancelled.update(tid for future, tid in futures.items()
                                 if tid not in results)
                for future in futures:
                    future.cancel()
            return cancelled, should_stop

    def execute_dag(self, tasks: list[dict[str, Any]],
                    aggregate: Optional[Callable[[dict[str, Any]], Any]] = None,
                    max_parallel: Optional[int] = None,
                    stop: Any = None, timeout: Optional[float] = None,
        ...
        fallos, coherente con la política de cuota del framework). ``stop`` es
        una señal cooperativa opcional y ``timeout`` limita el plazo del DAG;
        las tareas no iniciadas o pendientes quedan en ``cancelled``. Si se
        proporciona ``event_sink``, recibe eventos ``task`` con ``run_id``,
        ``task_id`` y estados ``queued``, ``running``, ``done``, ``error`` o
        ``cancelled``.
        """
        run_id = run_id or f"run-{uuid.uuid4().hex[:12]}"

        for task in tasks:
            _emit_task_event(event_sink, run_id, task["id"], "queued")
        ...
        cancelled: set[str] = set()
        for batch in order:
            if _dag_should_stop(stop, deadline):
                cancelled.update(batch)
                break
            runnable, skipped = _dag_batch_plan(batch, by_id, deps, failed)
            for tid in skipped:
                results[tid] = None
                failed.add(tid)
                _emit_task_event(event_sink, run_id, tid, "error",
                                 error="dependencia fallida")
            if not runnable:
                continue
            batch_cancelled, should_stop = self._execute_dag_batch(
                runnable, max_par, results, failed, deadline, stop, event_sink, run_id)
            cancelled.update(batch_cancelled)
            if should_stop:
                break
        if cancelled or (stop is not None and stop.is_set()):
            cancelled.update(t["id"] for t in tasks if t["id"] not in results)
        for tid in cancelled:
            results.setdefault(tid, None)
            _emit_task_event(event_sink, run_id, tid, "cancelled",
                             reason=getattr(stop, "reason", "timeout") if stop is not None else "timeout")
        out = {"results": results, "failed": sorted(failed),
               "cancelled": sorted(cancelled),
               "executed": len(results) - len(failed) - len(cancelled),
               "total": len(tasks)}
