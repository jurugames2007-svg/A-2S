...
class ToolRegistry:
    ...
    def _spawn_windows_shell(self, segment: str) -> subprocess.Popen:
        ...
        if posix_shell is None:
            if self.shell_unsafe:
                try:
                    return subprocess.Popen(
                        segment, stdin=subprocess.DEVNULL, stdout=subprocess.PIPE,
                        stderr=subprocess.PIPE, text=True, encoding="utf-8",
                        errors="replace", cwd=self.workspace, shell=True)
                except OSError as exc:
                    raise PermissionError(f"no se pudo invocar el comando: {exc}") from exc
            raise PermissionError(
    ...
    def shell(self, command: str, depth: int = 0) -> str:
        ...
        for segment in self._split_top(command, ";"):
            ...
            if not segment:
                continue
            if os.name == "nt" and find_posix_shell() is None and not self.shell_unsafe:
                native = self._native_windows_read(segment)
    ...
    def _native_windows_read(self, segment: str) -> Optional[str]:
        """Implementa dos consultas POSIX de solo lectura sin una shell."""
        find_match = re.fullmatch(
            r"find\s+\.\s+-type\s+f\s+-not\s+-path\s+(['\"])\./\.git/\*\1\s*\|\s*sort",
            segment,
        )
        if find_match:
            paths = []
            for root, _dirs, files in os.walk(self.workspace):
                if os.path.basename(root) == ".git" or ".git" + os.sep in root:
                    continue
                for name in files:
                    relative = os.path.relpath(os.path.join(root, name), self.workspace)
                    paths.append("./" + relative.replace(os.sep, "/"))
            return "\n".join(sorted(paths))
        ls_match = re.fullmatch(r"ls\s+([\w./*?\[\]-]+)", segment)
        if ls_match:
            pattern = os.path.join(self.workspace, ls_match.group(1))
            matches = sorted(os.path.basename(path) for path in glob.glob(pattern))
            return "\n".join(matches)
        return None
